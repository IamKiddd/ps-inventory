...
CreateThread(function()
	...
	local plates = {}
	local inventory = MySQL.Sync.fetchAll('SELECT count(item_id) as amount, item_id, id, name, information, slot, quality, MIN(creationDate) as creationDate FROM user_inventory2 group by item_id, slot, name')
	inv = {}
	for _, row in ipairs(plate) do
		table.insert(plates, row.plate)
	end
	local update_data = {}
	local query = "DELETE FROM user_inventory2 WHERE item_id = ? AND slot = ?"
	for i, row in ipairs(inventory) do
		...
		if QBCore.Shared.SplitStr(name, "-")[1] ~= "Trunk" and QBCore.Shared.SplitStr(name, "-")[1] ~= "GloveBox" and QBCore.Shared.SplitStr(name, "-")[1] ~= "Drop" then
			...
			if shouldDelete then
				inv[name] = nil
				update_data[#update_data + 1] = {  query = query, values = {row.item_id, row.slot  } }
			else
				table.insert(inv[name], row)
			end
		end
...
local function ChangeSlot(item, name, from, to, amount)
	MySQL.query.await('UPDATE user_inventory2 SET slot = ? WHERE name = ? AND slot = ? AND item_id = ? LIMIT ?' , {to, name, from, item,amount})
	local item = MySQL.Sync.fetchAll('SELECT count(item_id) as amount, item_id, id, name, information, slot, quality, MIN(creationDate) as creationDate FROM user_inventory2 WHERE name = ? group by item_id, slot, name', {name})
	inv[name] = item
end

local function ChangeInv(item, fromInv, fromSlot, toSlot, toInv, amount)
	MySQL.query.await('UPDATE user_inventory2 SET name = ?, slot = ? WHERE name = ? AND slot = ? AND item_id = ? LIMIT ?' , {toInv, toSlot, fromInv, fromSlot, item, amount})
	local item = MySQL.Sync.fetchAll('SELECT count(item_id) as amount, item_id, id, name, information, slot, quality, MIN(creationDate) as creationDate FROM user_inventory2 WHERE name = ? group by item_id, slot, name', {fromInv})
	inv[fromInv] = item
	local item2 = MySQL.Sync.fetchAll('SELECT count(item_id) as amount, item_id, id, name, information, slot, quality, MIN(creationDate) as creationDate FROM user_inventory2 WHERE name = ? group by item_id, slot, name', {toInv})
	inv[toInv] = item2
end
...
local function BuildInventory(inventory)
	...
	for _, item in pairs(inventory) do
		if item then
			local itemInfo = QBCore.Shared.Items[item.item_id:lower()]
			if itemInfo then
				...
				missingItems[#missingItems + 1] = item.item_id:lower()
			end
		end
	end
	...
	slot = tonumber(slot) or GetFirstSlotByItem(BuildInventory(inv['ply-'..Player.PlayerData.citizenid]), item, info)
 	if (totalWeight + (itemInfo['weight'] * amount)) <= Config.MaxInventoryWeight then
		if slot then
			local queries = {}
			for i = 1, amount, 1 do
				queries[#queries+1] = {query = 'INSERT INTO user_inventory2 (item_id, name, information, slot, dropped, creationDate, quality) VALUES (?, ?, ?, ?, ?, ?, ?)', values = {itemInfo['name'], 'ply-'..Player.PlayerData.citizenid, json.encode(info), slot, 0, os.time(), 100}}
			end
			local success = MySQL.transaction.await(queries)
			local item2 = MySQL.Sync.fetchAll('SELECT count(item_id) as amount, item_id, id, name, information, slot, quality, MIN(creationDate) as creationDate FROM user_inventory2 WHERE name = ? group by item_id, slot, name', {'ply-'..Player.PlayerData.citizenid})
			...
			for i = 1, Config.MaxInventorySlots, 1 do
				if BuildInventory(inv['ply-'..Player.PlayerData.citizenid])[i] == nil then
					local queries = {}
					for j = 1, amount, 1 do
						queries[#queries+1] = {query = 'INSERT INTO user_inventory2 (item_id, name, information, slot, dropped, creationDate, quality) VALUES (?, ?, ?, ?, ?, ?, ?)', values = {itemInfo['name'], 'ply-'..Player.PlayerData.citizenid, json.encode(info), i, 0, os.time(), 100}}
					end
					local success = MySQL.transaction.await(queries)
					local item2 = MySQL.Sync.fetchAll('SELECT count(item_id) as amount, item_id, id, name, information, slot, quality, MIN(creationDate) as creationDate FROM user_inventory2 WHERE name = ? group by item_id, slot, name', {'ply-'..Player.PlayerData.citizenid})
					inv['ply-'..Player.PlayerData.citizenid] = item2
...
local function RemoveItem(source, item, amount, slot)
	...
	slot = tonumber(slot)
	if slot then
		MySQL.query.await('DELETE FROM user_inventory2 WHERE name = ? AND slot = ? LIMIT ?' , {'ply-'..Player.PlayerData.citizenid, slot, amount})
		local item = MySQL.Sync.fetchAll('SELECT count(item_id) as amount, item_id, id, name, information, slot, quality, MIN(creationDate) as creationDate FROM user_inventory2 WHERE name = ? group by item_id, slot, name', {'ply-'..Player.PlayerData.citizenid})
		...
		for _, _slot in pairs(slots) do
			if BuildInventory(inv['ply-'..Player.PlayerData.citizenid])[_slot].amount >= amount then
				MySQL.query.await('DELETE FROM user_inventory2 WHERE name = ? AND slot = ? LIMIT ?' , {'ply-'..Player.PlayerData.citizenid, _slot, amount})
				inv['ply-'..Player.PlayerData.citizenid] = item
				if Player.Offline then return true end
				...
				MySQL.query.await('DELETE FROM user_inventory2 WHERE name = ? AND slot = ? LIMIT ?' , {'ply-'..Player.PlayerData.citizenid, _slot, BuildInventory(inv['ply-'..Player.PlayerData.citizenid])[_slot].amount})
...
local function ClearInventory(source, filterItems)
	...
	if filterItems then
		...
		if filterItemsType == "string" then
			MySQL.query.await('UPDATE user_inventory2 SET name = ? WHERE name = ? AND item_id = ?' , {'saveply-'..Player.PlayerData.citizenid, 'ply-'..Player.PlayerData.citizenid, filterItems})
		elseif filterItemsType == "table" and table.type(filterItems) == "array" then
			local update_data = {}
			local query = "UPDATE user_inventory2 SET name = ? WHERE name = ? AND item_id = ?"
		...
		MySQL.query.await('DELETE FROM user_inventory2 WHERE name = ?' , {'ply-'..Player.PlayerData.citizenid})
	end
	MySQL.query.await('UPDATE user_inventory2 SET name = ? WHERE name = ?' , {'ply-'..Player.PlayerData.citizenid, 'saveply-'..Player.PlayerData.citizenid})
	inv['ply-'..Player.PlayerData.citizenid] = MySQL.Sync.fetchAll('SELECT count(item_id) as amount, item_id, id, name, information, slot, quality, MIN(creationDate) as creationDate FROM user_inventory2 WHERE name = ? group by item_id, slot, name', {'ply-'..Player.PlayerData.citizenid})
...
RegisterNetEvent('inventory:server:SetInventoryData', function(fromInventory, toInventory, fromSlot, toSlot, fromAmount, toAmount)
	...
	if fromInventory == "player" or fromInventory == "hotbar" then
		...
		if fromItemData and fromItemData.amount >= fromAmount then
			...
			elseif QBCore.Shared.SplitStr(toInventory, "-")[1] == "otherplayer" then
				local playerId = tonumber(QBCore.Shared.SplitStr(toInventory, "-")[2])
				local OtherPlayer = QBCore.Functions.GetPlayer(playerId)
				local toItemData = OtherBuildInventory(inv['ply-'..Player.PlayerData.citizenid])[toSlot]
	...
	elseif QBCore.Shared.SplitStr(fromInventory, "-")[1] == "otherplayer" then
		local playerId = tonumber(QBCore.Shared.SplitStr(fromInventory, "-")[2])
		local OtherPlayer = QBCore.Functions.GetPlayer(playerId)
		local fromItemData = OtherBuildInventory(inv['ply-'..Player.PlayerData.citizenid])[fromSlot]
		fromAmount = tonumber(fromAmount) or fromItemData.amount
		if fromItemData and fromItemData.amount >= fromAmount then
			local itemInfo = QBCore.Shared.Items[fromItemData.name:lower()]
			if toInventory == "player" or toInventory == "hotbar" then
